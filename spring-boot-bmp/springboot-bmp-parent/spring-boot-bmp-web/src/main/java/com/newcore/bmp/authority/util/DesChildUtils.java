...
public class DesChildUtils {
    
    private DesChildUtils(){
        super();
    }
    /*
    ...
    public static int[] strToBt(String str) {
        ...
        if (leng < 4) {
            for (int i = 0; i < leng; i++) {
                int k = str.charAt(i);
                for (int j = 0; j < 16; j++) {
                    bt[16 * i + j] = getPow(j, k);
                }
            }
            for (int p = leng; p < 4; p++) {
                int k = 0;
                for (int q = 0; q < 16; q++) {
                    bt[16 * p + q] = getPow(q, k);
                }
            }
        } else {
            for (int i = 0; i < 4; i++) {
    ...
    private static int getPow(int j, int k) {
        int pow = 1;
        for (int m = 15; m > j; m--) {
            pow *= 2;
        }
    ...
    public static String bt64ToHex(int[] byteData) {
        ...
        for (int i = 0; i < 16; i++) {
            String bt = "";
            for (int j = 0; j < 4; j++) {
                int byteDataInt = byteData[i * 4 + j];
                bt += Integer.toString(byteDataInt);
            }
            hex += bt4ToHex(bt);
        }
        return hex;
    ...
    private static int[] sBoxPermute(int[] expandByte) {
        ...
        for (int m = 0; m < 8; m++) {
            ...
            int j = expandByte[m * 6 + 1] * 2 * 2 * 2 + expandByte[m * 6 + 2] * 2 * 2 + expandByte[m * 6 + 3] * 2
                    + expandByte[m * 6 + 4];
