...
public class TaskServiceImpl {
				...
				for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.<JobKey>groupEquals(groupJob))) {
					...
					for (Trigger trigger : triggers) {
						...
						if (trigger instanceof CronTrigger) {
							...
							info.setNextTime(DateUtil.format(cronTrigger.getNextFireTime(), "yyyy-MM-dd HH:mm:ss"));
						}
	...
	public void addJob(TaskBaseInfo info) {
		String jobName = info.getJobName(), jobGroup = info.getJobGroup(), jobClass = info.getJobClass(),
		...
		try {
			...
			JobDetail jobDetail = JobBuilder.newJob(clazz).withIdentity(jobKey).withDescription(jobDescription)
					.setJobData(new JobDataMap(info.getJobDataMap())).build();
	 ...
	 *            2016年10月9日下午2:20:07
	 */
	public void edit(TaskBaseInfo info) {
